...
class Solution:
    ...
    #    对于情况二，分别递归左孩子和右孩子，递归到某一深度一定会有左孩子或者右孩子为满二叉树，然后依然可以 按照情况1来计算。
    def countNodes3(self, root: TreeNode) -> int:
        ...
        while right:  # 求右子树深度
            right = right.right
            rightHeight += 1
        if leftHeight == rightHeight:  # 情况一：满二叉树
            # 注意(2 << 1) 相当于2^2，所以leftHeight初始为0
            return (2 << leftHeight) - 1
        return self.countNodes3(root.left) + self.countNodes3(root.right) + 1  # 情况二
