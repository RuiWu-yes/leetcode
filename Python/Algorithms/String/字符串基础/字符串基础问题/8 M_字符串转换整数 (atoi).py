...
#                5) 如果整数数超过 32 位有符号整数范围 [−2^31, 2^31 − 1] ，需要截断这个整数，使其保持
#                   在这个范围内。具体来说，小于 −2^31 的整数应该被固定为 −2^31 ，大于 2^31 − 1 的整数
#                   应该被固定为 2^31 − 1 。
#                6) 返回整数作为最终结果。
#       注意 : 本题中的空白字符只包括空格字符 ' ' 。
...
class Solution:
    def myAtoi(self, s: str) -> int:
        ls = list(s.strip())  # 去除前导空格
        if len(ls) == 0:
            return 0
        else:
            sign = -1 if ls[0] == '-' else 1  # 检测是否含有负号
        if ls[0] in ['-', '+']: del ls[0]  # 去除起始的'-'或者'+'字符
        # 遇到字符串数字转换成数字
        res, i = 0, 0
